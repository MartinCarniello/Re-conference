...
Given(/^seteo el rol "(.*?)"$/) do |rol|
	@browser.select_list(:id, "account_rol_de_usuario").select_value(rol)
end

Given(/^ingreso "(.*?)" como nombre de usuario$/) do |nombre|
	@browser.text_field(id: "account_nombre").set nombre
end
...
Given(/^confirmo la contrasenia "(.*?)"$/) do |confirm|
	@browser.text_field(id: "account_confirme_password").set confirm
end
